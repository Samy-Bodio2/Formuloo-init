...
class FichePaieCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_employe_id(self, value):
        ...
        if not Employe.objects.filter(
            ...
            raise serializers.ValidationError(
                "L'employé sélectionné est introuvable ou n'est plus actif. "
                "Vérifiez que l'employé existe et est en statut actif."
            )
        return value
    ...
    def validate_contrat_id(self, value):
        ...
        if not Contrat.objects.filter(
            ...
            raise serializers.ValidationError(
                "Le contrat sélectionné est introuvable ou n'est plus actif."
            )
        return value
    ...
    def update(self, instance, validated_data):
        ...
        if instance.statut != FichePaie.Statut.BROUILLON:
            raise serializers.ValidationError(
                "Cette fiche de paie est déjà validée ou payée et ne peut plus être modifiée. "
                "Seules les fiches en brouillon sont modifiables."
            )
