...
class EmployeCreateSerializer(serializers.ModelSerializer):
    ...
    user_id = serializers.UUIDField(
        ...
        help_text="UUID du compte auth lié (optionnel — permet à l'employé de soumettre des demandes)"
    )
    ...
    def validate_position_id(self, value):
        ...
        if not Poste.objects.filter(
            ...
            raise serializers.ValidationError(
                "Le poste sélectionné est introuvable ou désactivé. "
                "Vérifiez la liste des postes actifs."
            )
        return value
    ...
    def validate_manager_id(self, value):
        ...
        if not Employe.objects.filter(
            ...
            raise serializers.ValidationError(
                "Le manager sélectionné est introuvable ou n'est plus actif dans l'organisation."
            )
        return value
        ...
        if position_id and department_id:
            try:
                ...
                if str(poste.departement_id) != str(department_id):
                    raise serializers.ValidationError(
                        "Le poste sélectionné n'appartient pas au département choisi. "
                        "Veuillez choisir un poste rattaché à ce département."
                    )
