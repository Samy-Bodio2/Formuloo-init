...
class DepartementCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, value):
        ...
        if qs.exists():
            raise serializers.ValidationError(
                "Ce code est déjà utilisé par un autre département. Choisissez un code différent."
            )
        return value
    ...
    def validate_parent_id(self, value):
        ...
        if not Departement.objects.filter(id=value, tenant_id=tenant_id).exists():
            raise serializers.ValidationError(
                "Le département parent sélectionné est introuvable dans votre organisation."
            )
        return value
