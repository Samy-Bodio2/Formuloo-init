...
class TestRoles:
    ...
    def test_lister_roles_succes(self, client_alpha, role_admin_alpha, role_rh_alpha):
        """
        Given : deux rôles custom dans PME Alpha + les 7 rôles système globaux (seedés
                par la migration 0009, partagés par tous les tenants)
        When  : l'admin liste les rôles
        Then  : il voit ses 2 rôles custom + les 7 rôles système — 200
        """
        response = client_alpha.get("/api/v1/auth/roles/")

        assert response.status_code == 200
        codes = [r["id"] for r in response.data]
        assert str(role_admin_alpha.id) in codes
        assert str(role_rh_alpha.id) in codes
