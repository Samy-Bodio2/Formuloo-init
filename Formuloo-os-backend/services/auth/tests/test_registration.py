...
class TestAcceptInvitation:
    ...
    def test_accept_success(self, client_anon, invited_user):
        """Code valide + mot de passe → compte activé."""
    ...
    def test_accept_invalid_code(self, client_anon):
        """Code inexistant → 400."""
        resp = client_anon.post(
            self.URL,
            {
...
# ── TESTS INVITE PREVIEW ──────────────────────────────────────────────────────


@pytest.mark.django_db
class TestInvitePreview:
    """GET /invite/preview/?code=123456 — aperçu d'une invitation avant acceptation."""

    def _invite_url(self, code):
        return f"{BASE}/invite/preview/?code={code}"

    def test_preview_valid_code(self, client_rh, client_anon, org, role_employe):
        """Code valide → 200 avec organisation/rôle/inviteur."""
        client_rh.post(
            f"{BASE}/invite/",
            {
                "first_name": "Awa",
                "last_name": "Traore",
                "email": "awa.preview@pme.com",
                "roles": ["EMPLOYE"],
            },
            format="json",
        )
        invited_user = User.objects.get(email="awa.preview@pme.com")
        ott = OneTimeToken.objects.get(
            user=invited_user, token_type=OneTimeToken.Type.INVITATION
        )

        resp = client_anon.get(self._invite_url(ott.code))
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data["email"] == "awa.preview@pme.com"
        assert resp.data["first_name"] == "Awa"
        assert resp.data["last_name"] == "Traore"
        assert resp.data["organisation"]["name"] == org.name
        assert resp.data["role"] == role_employe.name
        assert resp.data["invited_by"] == "Alice Martin"
        assert "expires_at" in resp.data

    def test_preview_does_not_consume_code(self, client_rh, client_anon):
        """L'aperçu ne doit pas invalider le code — l'acceptation doit rester possible."""
        client_rh.post(
            f"{BASE}/invite/",
            {
                "first_name": "Bakary",
                "last_name": "Sow",
                "email": "bakary.preview@pme.com",
                "roles": ["EMPLOYE"],
            },
            format="json",
        )
        invited_user = User.objects.get(email="bakary.preview@pme.com")
        ott = OneTimeToken.objects.get(
            user=invited_user, token_type=OneTimeToken.Type.INVITATION
        )

        client_anon.get(self._invite_url(ott.code))
        resp = client_anon.post(
            f"{BASE}/invite/accept/",
            {
                "code": ott.code,
                "password": "Pass2024!",
                "confirm_password": "Pass2024!",
    ...
    def test_preview_unknown_code(self, client_anon):
        """Code inexistant → 404."""
        resp = client_anon.get(self._invite_url("000000"))
        assert resp.status_code == status.HTTP_404_NOT_FOUND

    def test_preview_used_code(self, client_anon, org, role_employe):
        """Code déjà consommé → 400."""
        user = User.objects.create_user(
            email="used.preview@pme.com",
            tenant=org,
            password=None,
            first_name="Used",
            last_name="Token",
    ...
    def test_preview_expired_code(self, client_anon, org, role_employe):
        """Code expiré → 400."""
        user = User.objects.create_user(
            email="expired.preview@pme.com",
            tenant=org,
            password=None,
            first_name="Expired",
            last_name="Token",
        ...
        ott = OneTimeToken.create_for(user, OneTimeToken.Type.INVITATION)
        ott.expires_at = timezone.now() - timezone.timedelta(hours=1)
        ott.save(update_fields=["expires_at"])
...
class TestResetPassword:
    ...
    def test_reset_success(self, client_anon, rh_manager):
        """Code valide + nouveau mdp → mdp modifié."""
    ...
    def test_reset_code_single_use(self, client_anon, rh_manager):
        """Le code de reset ne peut être utilisé qu'une fois."""
        ott = OneTimeToken.create_for(rh_manager, OneTimeToken.Type.RESET_PASSWORD)
        payload = {
            "code": ott.code,
            "nouveau_mot_de_passe": "NouveauMDP2024!",
    ...
    def test_reset_expired_code(self, client_anon, rh_manager):
        """Code expiré → 400."""
        ott = OneTimeToken.create_for(rh_manager, OneTimeToken.Type.RESET_PASSWORD)
