...
class VerifyEmailSerializer(serializers.Serializer):
    """
    Vérifie l'adresse email d'un utilisateur via le token
    reçu par email après l'inscription ou après renvoi.
    """

    token = serializers.UUIDField(
        help_text="Token UUID reçu dans le lien de vérification email",
    )

    def validate_token(self, value) -> OneTimeToken:
        """
        Cherche et valide le token VERIFY_EMAIL.
...
class AcceptInvitationSerializer(serializers.Serializer):
    ...
    token = serializers.UUIDField(
        help_text="Token UUID reçu dans le lien d'invitation email",
    )
    password = serializers.CharField(
        write_only=True,
        min_length=8,
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    token = serializers.UUIDField(
        help_text="Token UUID reçu dans le lien de réinitialisation email",
    )
    ...
    def validate_token(self, value) -> OneTimeToken:
        """Valide le token RESET_PASSWORD (2h d'expiry)."""
        try:
            ott = OneTimeToken.objects.select_related("user").get(
                token=value,
