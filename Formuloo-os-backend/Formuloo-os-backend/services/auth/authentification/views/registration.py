...
from authentification.models import AuditLog, OneTimeToken, Role, User
...
def _send_invitation_email(user: User, invited_by: User):
    """
    Génère un token INVITATION et envoie le lien d'activation
    à l'employé invité.

    Le lien pointe vers FRONTEND_URL/invite/accept?token=<uuid>.
    ...
    invite_url = f"{settings.FRONTEND_URL}/invite/accept?token={ott.token}"
    ...
    send_mail(
        ...
        message=(
            ...
            f"Cliquez sur ce lien pour créer votre mot de passe et activer votre compte :\n"
            f"{invite_url}\n\n"
            f"Ce lien expire dans 72h.\n\n"
            f"L'équipe Formuloo OS"
        ),
        from_email=settings.DEFAULT_FROM_EMAIL,
        recipient_list=[user.email],
...
def _send_reset_email(user: User):
    """
    Génère un token RESET_PASSWORD et envoie le lien de reset.

    Le lien pointe vers FRONTEND_URL/reset-password?token=<uuid>.
    Expire après 2h.
    ...
    reset_url = f"{settings.FRONTEND_URL}/reset-password?token={ott.token}"
    ...
    send_mail(
        ...
        message=(
            ...
            f"Cliquez sur ce lien (valable 2h) :\n"
            f"{reset_url}\n\n"
...
class VerifyEmailView(APIView):
    ...
    Valide l'adresse email via le token reçu par email.
    ...
    @extend_schema(
        ...
        description=(
            ...
            "Le token est valide 48h et ne peut être utilisé qu'une seule fois."
        ),
        tags=["Inscription & Compte"],
    ...
    def post(self, request):
        ...
        # Le serializer a déjà résolu le token → OneTimeToken instance
        ott = serializer.validated_data["token"]
        user = ott.user
...
class InviteView(APIView):
    ...
    def post(self, request):
        ...
        _send_invitation_email(invited_user, invited_by=request.user)
