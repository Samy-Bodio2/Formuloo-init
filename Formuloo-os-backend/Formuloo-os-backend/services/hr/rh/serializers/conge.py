...
class CongeCreateSerializer(serializers.ModelSerializer):
        ...
        if start_date and end_date:
            ...
            try:
                ...
                if qs.exists():
                    ...
                    raise serializers.ValidationError(
                        f"Ce congé chevauche un congé "
                        f"existant du "
                        f"{conge_existant.start_date} "
                        f"au {conge_existant.end_date} "
                        f"(statut: {conge_existant.status})."
                    )
    ...
    def validate_remplacant_id(self, value):
        ...
        if not Employe.objects.filter(
            ...
            raise serializers.ValidationError("Remplaçant introuvable ou inactif.")
        return value
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Aucun profil employé " "lié à votre compte."
            )
